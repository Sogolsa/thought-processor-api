...
var _gratitudeControllers = require("../controllers/gratitudeControllers");
...
var routes = function routes(app) {
  ...
  (0, _expressValidator.check)("userName", "userName is required").isLength({
    min: 1
  }), (0, _expressValidator.check)("userName", "userName contains non alphanumeric characters-not allowed.").isAlphanumeric(), (0, _expressValidator.check)("Password").isLength({
    min: 8
  }).withMessage("Password must be at least 8 characters long.").matches(/[a-z]/).withMessage("Password must contain at least one lowercase letter.").matches(/[A-Z]/).withMessage("Password must contain at least one uppercase letter.").matches(/\d/).withMessage("Password must contain at least one number.").matches(/[\W_]/).withMessage("Password must contain at least one special character.").not().isEmpty().withMessage("Password is required."), (0, _expressValidator.check)("Email", "Email does not appear to be valid.").isEmail()], _userControllers.registerUser);
  ...
  app.route("/gratitude").post(_passport["default"].authenticate("jwt", {
    session: false
  }), _gratitudeControllers.addGratitude).get(_passport["default"].authenticate("jwt", {
    session: false
  }), _gratitudeControllers.getGratitudeEntries);
  app.route("/gratitude/:gratitudeId").get(_passport["default"].authenticate("jwt", {
    session: false
  }), _gratitudeControllers.getGratitudeById).put(_passport["default"].authenticate("jwt", {
    session: false
  }), _gratitudeControllers.updateGratitude)["delete"](_passport["default"].authenticate("jwt", {
    session: false
  }), _gratitudeControllers.deleteGratitude);
